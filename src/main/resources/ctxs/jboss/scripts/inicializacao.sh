...
if [ "$flyway_run" == "on" -o "$flyway_run" == "auto" ]
        then
                ...
                unzip -j /opt/jboss/sigadoc/deployments/siga.war WEB-INF/lib/siga-cp-1.2-SNAPSHOT.jar  -d /siga/flyway-3.0/siga-updates
                unzip  /siga/flyway-3.0/siga-updates/siga-cp-1.2-SNAPSHOT.jar db/migration/* -d /siga/flyway-3.0/sql/corporativo/

                unzip -j /opt/jboss/sigadoc/deployments/sigaex.war WEB-INF/lib/siga-ex-1.2-SNAPSHOT.jar -d /siga/flyway-3.0/siga-updates
                unzip  /siga/flyway-3.0/siga-updates/siga-ex-1.2-SNAPSHOT.jar  db/migration/* -d /siga/flyway-3.0/sql/siga/
                ...
                unzip -j /opt/jboss/sigadoc/deployments/sigagc.war WEB-INF/classes/db/* -d /siga/flyway-3.0/siga-updates/sigagc
...
if [ "$flyway_run" == "off" -o "$flyway_run" == "auto" ]
        then
              /opt/jboss/bin/init.d/sigadoc.sh start
fi
