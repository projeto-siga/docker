...
docker run -d --name bluc.server -h bluc.server -p 50010:8080 siga/bluc.server:release-7.0.3

docker run -d --name app.server -h app.server -m=2g --link db.server:db.server --link email.server:email.server --link bluc.server:bluc.server --link viz.server:viz.server -e init_db=on -e flyway_run=auto -p 50000:8080 -p 9990:9990 siga/app.server:release-7.0.3

echo "Iniciando web.server..."
docker run -d --name web.server -h web.server -p 80:80 --link app.server:app.server siga/web.server:release-7.0.3
