...
docker run -d --name app.server -h app.server -m=2g --link db.server:db.server --link email.server:email.server --link bluc.server:bluc.server -e init_db=on -e flyway_run=auto -p 50000:8080 -p 9990:9990 siga/app.server
