...
docker run -d --name app.server -h app.server -m=1g --link db.server:db.server --link email.server:email.server -e init_db=on -e flyway_run=auto -p 50000:8080 siga/app.server
